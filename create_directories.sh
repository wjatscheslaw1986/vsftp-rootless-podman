...
if [ "$(id -u)" = 0 ]; then
    ...
    chown -R "${VSFTPD_ROOT_OWNERSHIP}" "${BASE_DIR}/ftp"
    ...
    echo "You need to execute 'mkdir -p "${BASE_DIR}"/{ftp,mariadb/data,log/{vsftpd,mariadb}}' with root privileges, presumably" 
